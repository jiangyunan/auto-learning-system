...
class TestSummarizerL1:
    ...
    @pytest.mark.asyncio
    async def test_generate_l1_basic(self, mock_llm, mock_cache, features_enabled, sample_chunk):
        """测试基本的L1生成"""
    ...
    @pytest.mark.asyncio
    async def test_generate_l1_empty_response(self, mock_llm, mock_cache, features_enabled, sample_chunk):
        """测试L1空响应处理"""
        mock_llm.complete_json = AsyncMock(return_value={})
    ...
    @pytest.mark.asyncio
    async def test_generate_l1_error_handling(self, mock_llm, mock_cache, features_enabled, sample_chunk):
        """测试L1错误处理"""
...
class TestSummarizerL2:
    ...
    @pytest.mark.asyncio
    async def test_generate_l2_when_enabled(self, mock_llm, mock_cache, features_enabled, sample_chunk):
        """测试启用中文笔记时生成L2"""
    ...
    @pytest.mark.asyncio
    async def test_skip_l2_when_disabled(self, mock_llm, mock_cache, features_disabled, sample_chunk):
        """测试禁用时跳过L2"""
        mock_llm.complete_json = AsyncMock(return_value={
            "bullets": ["Point 1"],
            "key_concepts": [],
    ...
    @pytest.mark.asyncio
    async def test_l2_error_handling(self, mock_llm, mock_cache, features_enabled, sample_chunk):
        """测试L2错误处理"""
        mock_llm.complete_json = AsyncMock(side_effect=[
...
class TestSummarizerCache:
    ...
    @pytest.mark.asyncio
    async def test_cache_hit(self, mock_llm, mock_cache, features_enabled, sample_chunk):
        """测试缓存命中"""
        mock_cache.get = Mock(return_value={
            "l1": {"bullets": ["Cached point"], "key_concepts": ["Cached"]},
            "l2": {"overview": "Cached overview", "key_points": [], "concepts_explained": [], "code_examples": [], "related_topics": []},
        })
    ...
    @pytest.mark.asyncio
    async def test_cache_miss_saves(self, mock_llm, mock_cache, features_enabled, sample_chunk):
        """测试缓存未命中时保存"""
        mock_llm.complete_json = AsyncMock(return_value={
            "bullets": ["Point 1"],
            "key_concepts": [],
...
class TestMergeSummaries:
    ...
    async def test_merge_l2_summaries(self, mock_llm, mock_cache, features_enabled):
        """测试L2摘要合并"""
        summaries = [
            SummaryL2(overview="Overview 1", key_points=["A"], concepts_explained=[{"term": "T1"}]),
            SummaryL2(overview="Overview 2", key_points=["B"], concepts_explained=[{"term": "T2"}]),
        ]
